...
public final class Startup_sA1GYNviEeWsF7MALrkAZQ implements DIMEProcess {
	...
	private Startup_sA1GYNviEeWsF7MALrkAZQResult executeInternal(final Context ctx, final SIB_ID id) {
		...
		while (true) {
			switch (curr) {
				...
				case _VGBlQWl_Ee2DwNdDojdu8A: {
					curr = execute_VGBlQWl_Ee2DwNdDojdu8A(ctx);
					break;
				}
				default: {
	...
	public SIB_ID execute_PXMhsNzNEeWtAOZsxPLKSA(final Context ctx) {
		...
		if (result.isEmpty()) {
			...
			// branch 'success'
			return SIB_ID._VGBlQWl_Ee2DwNdDojdu8A;
		}
	}
	...
	// container for graph abstraction 'ErrorProcess' and sub process 'ErrorProcess'.
	public SIB_ID execute_VGBlQWl_Ee2DwNdDojdu8A(final Context ctx) {
		final info.scce.dime.process.dime__HYPHEN_MINUS__models.interaction.ErrorProcess_2gSyEWl__HYPHEN_MINUS__Ee2DwNdDojdu8A instance = new info.scce.dime.process.dime__HYPHEN_MINUS__models.interaction.ErrorProcess_2gSyEWl__HYPHEN_MINUS__Ee2DwNdDojdu8A(ctx.beanManager);
	
		final info.scce.dime.process.dime__HYPHEN_MINUS__models.interaction.ErrorProcess_2gSyEWl__HYPHEN_MINUS__Ee2DwNdDojdu8A.ErrorProcess_2gSyEWl__HYPHEN_MINUS__Ee2DwNdDojdu8AResult result = instance.execute(false);
		...
		case "success": {
			...
			default: throw new IllegalStateException("SIB 'ErrorProcess' has no successor defined for branch '" + result.getBranchName() + '\'');
		}
	}
